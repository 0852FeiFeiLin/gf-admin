...
import (
	...
	"github.com/SupenBysz/gf-admin/utility/response"
	"time"
...
func (s *sJwt) GenerateToken(user *entity.SysUser) (*model.TokenInfo, error) {
	customClaims := model.JwtCustomClaims{
		Id:        user.Id,
...
func (s *sJwt) CreateToken(claims *model.JwtCustomClaims) (string, error) {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)
	return token.SignedString(s.SigningKey)
}

// RefreshToken 刷新Token,并发安全
func (s *sJwt) RefreshToken(oldToken string, claims *model.JwtCustomClaims) (string, error) {
...
func (s *sJwt) CustomMiddleware(r *ghttp.Request) {
	...
	service.BizCtx().SetUser(r.Context(), claimsUser)
}
...
func (s *sJwt) ParseToken(tokenString string) (*model.JwtCustomClaims, error) {
	if gstr.HasPrefix(tokenString, "Bearer ") {
		tokenString = gstr.SubStr(tokenString, 7)
	}

	token, err := jwt.ParseWithClaims(tokenString, &model.JwtCustomClaims{}, func(token *jwt.Token) (i interface{}, e error) {
